...
namespace App\Filament\Resources\RamUsageChartResource\Widgets;
...
{
    protected static ?string $heading = 'RAM Usage by DBNAME';
    ...
    protected function getData(): array
    {
        ...
        foreach ($metrics as $env => $records) {
            ...
            foreach ($records->sortBy('timestamp') as $record) {
                ...
                // Ambil nilai RAM dari format "(xx%)"
                preg_match('/\((.*?)%\)/', $record->memory_usage, $match);
                $ramData[] = isset($match[1]) ? floatval($match[1]) : 0;
            }

            $color = $env ? FilteredMetric::getEnvironmentColor($env) : 'rgba(128, 128, 128, 0.5)';
    ...
    {
        return '20s'; // Auto-refresh setiap 60 detik
    }
}